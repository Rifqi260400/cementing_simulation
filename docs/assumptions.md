...
| A-54 | Dai et al. (2024) Eq. 4, A.10–A.18 | Two gravity mechanisms driven by `v_t = √(At·g·sin β·D)`: density segregation of the cross-section when `Re_t > 1`, and instantaneous mixing when one of four instability criteria fires | Nothing — they are fully specified, with every coefficient from a cited experiment | **Implemented** in `inpipe/segregation.py`, applied per station each step | **This is the buoyancy the paper actually has, and it needs no calibration.** `β` is measured *from vertical*, so on a vertical well `sin β = 0`, `v_t = 0`, and both mechanisms are inert — by geometry, not approximation: gravity has no component across a horizontal cross-section to stratify it. Verified: `v_t` = 0.000 (vertical), 0.306 (30°), 0.433 m/s (horizontal); segregation `Re_t` = 0 vs 290. The vertical results are bit-identical with it wired in. It earns its place by making a deviated section modellable with no new physics, and conservation survives it (sum-to-one 9.3e-13 at 45°). Their rule that buoyancy instabilities are "absent for density-stable flow" is honoured, and a slow viscous section is stable whichever fluid is on top — at the job's 0.277 m/s, Re = 27 and no criterion fires either way | yes — 0°, 45°, 90° |
| A-55 | — | — | **Correction to A-54's predecessor.** An earlier entry claimed transverse buoyancy of `V_b = 0.709 m/s` against an imposed 0.277 on this well, i.e. 2.5× the flow | — | **Retracted.** That used the full `g` across the gap — a horizontal-well formula on a vertical well. Dai et al.'s `sin β` is exactly the missing factor; the transverse buoyancy here is **zero** | What survives in a vertical well is different and neither paper models it: gravity is axial while the composition gradient across the gap is transverse, so buoyancy cannot segregate — but the front is not flat, so inside the mixing zone the cement-rich core carries a larger body force than the mud-rich wall and is retarded relative to it, **flattening the front**. Magnitude: the differential body force is **6590 Pa/m against a frictional gradient of 748 Pa/m, a factor of 8.8**. So it is not a correction, it is dominant *within the mixing zone* — and ANSYS will capture it because it solves `ρg` in the momentum equation regardless of which way the density gradient points. Direction of the expected disagreement: **shorter interface and higher efficiency in the CFD**; arrival time much closer, being set by volume balance | yes — pinned by tests so the old formula cannot return |
| A-52 | — | Reduced-order modelling generally, this model included | The velocity profile is the **fully developed** one for the local gap, solved as though the passage went on forever | Keep it — there is no alternative without a momentum equation — but **measure where it fails and say so** (`FlowRegime.developing`) | Quantified on K-GEP-1: the laminar entrance length `L_e = 0.05 Re D_h` is **1.78 m** (median) while the caliper changes the section over `|D_h/(dD_h/dz)|` of **2.98 m** (median) but as little as **0.14 m** at the p1 sharpest steps. **Over 41 % of the interval `L_e > L_geom`** — the flow is still adjusting to the last change in section when the next arrives — and the ratio reaches 27 at the worst washout edges. This is the structural limit of the whole reduced-order approach, not a defect of this implementation, and it is where the model and the CFD should be *expected* to disagree. It is also the mechanism behind the flow separation visible in the ANSYS contours. **The flagged fraction rises under mesh refinement** — 0.0 % at 60 axial stations, 2.4 % at 125, 4.8 % at 250, against 41 % measured on the caliper at its own 1 cm sampling. Refining does not converge this assumption, it *exposes* it: a coarse grid hides the violation by smoothing the hole it cannot resolve. So the 5 % the solver reports on its own grid is optimistic and the 41 % on the log is the honest figure for the real well. **Practical consequence: aim the ANSYS comparison at the washout edges, not the smooth sections** — the smooth majority is where the reduced-order model is already trustworthy | yes — flagged per station on every run, plus a mesh sweep |
| A-53 | — | — | What the numerical diffusion should be compared against | The **shear** spreading the model itself resolves, not a diffusivity | An earlier version of `tests/test_transport.py` printed a ratio to a "physical Dm ~ 1e-3 m²/s" — a number with nothing behind it. The physical diffusivity here is molecular, ~1e-9 m²/s, so the numerical value is ~4 orders of magnitude *larger*; and Taylor–Aris dispersion, which would raise the physical figure, is **not established** — it needs a radial mixing time `R²/D_m` of about 22 days against a job of minutes. The mechanism that actually spreads the front is shear, which the model resolves. On the field case numerical smearing gives **12.6 m** of front width against a measured interface length of **108 m**: 12 % in length, **1.3 % in variance**. Subdominant — the same conclusion as before, but reached correctly | yes |
| A-50 | Xue et al. (2022) *J. Pet. Sci. Eng.* 208:109393 | Wide side turbulent, narrow side laminar in a cementing annulus; assuming one regime everywhere "will lead to serious model error" | This solver integrates a **laminar** Herschel–Bulkley profile everywhere and has no turbulence model | **Laminar everywhere, and the check that justifies it runs on every job.** `inpipe/regime.py` computes `Re = ρVD_h/μ_eff` with `μ_eff = τ_w/γ̇(τ_w)` from the solver's own law, per station, at every diagnostic step | Checked rather than assumed, and it passes. Both fluids are Herschel–Bulkley and the ANSYS comparison will use Herschel–Bulkley too, so the HB numbers are the relevant ones: at the job rate of 5 bpm, Re is **404 (mud) / 412 (cement) in the annulus and 822–875 in the casing** — a factor of five below the laminar limit in the annulus, 2.4 in the casing. The **casing leaves laminar first**, at roughly 10 bpm (Re 2344); the annulus holds to 15–20 bpm. Two conditions would need a re-check: a higher pump rate, or a displaced fluid far thinner than the placeholder mud — water at 1 cP gives Re ≈ 38 000, which bounds the sensitivity. Reported, never modelled: a turbulence model is a different solver | yes — rate sweep 5–60 bpm for both fluids, and a test pins the flag |
