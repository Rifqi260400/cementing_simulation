...
| A-54 | Xue et al. (2022); Tao et al. (2025); any Fluent run with gravity on | Buoyancy is in the `ρg` term of the momentum equation, and Xue et al. call it one of the two strongest influences on the interface | This model has **no momentum equation**, so density never drives the flow (A-29) — but the CFD it will be validated against does | **Keep the omission, measure it, and predict its direction** (`inpipe/buoyancy.py`, reported on every run) | Not a small correction: `V_b = sqrt(g'h)` is **0.709 m/s in the annulus against an imposed 0.277** (Fr = 0.39, so 2.5x the flow) and 0.821 vs 0.684 in the casing (Fr = 0.83) — buoyancy outweighs the flow in **both** legs, and a blob crosses the annular gap in 0.17 s, some 6800 times during the job. The two legs are in **opposite** stratifications and both bias the same way: the annulus is stable (cement below the mud it lifts), so buoyancy flattens the interface this model has no way to flatten; the casing is unstable (cement above), so buoyancy runs the front ahead. **Falsifiable prediction: ANSYS should show a shorter interface, higher efficiency and earlier arrival.** If it shows a *longer* interface, something other than buoyancy is wrong. **No drift-flux closure was added**: it needs a coefficient nothing in this study constrains, and a mis-calibrated buoyancy model is harder to detect than an absent one. The right order is to let the CFD measure the effect on the matched case and calibrate against it — which turns the validation runs into something that improves this model rather than only grading it | yes — scales reported per leg, direction pinned by tests |
